...
class CustomScrollView : UIScrollView  {
    ...
    public func addContentViews(With views : [UIView]){
        ...
        self.resizeContentInset()
        
    }
    /// 重新計算 ScrollView 的 ContentInset
    /// ## 在 Viewcontroller 的 viewDidLayoutSubviews() 中執行
    public func resizeContentInset(){
        self.delegate = self
    ...
    public func startAutoSlideShow(With time : TimeInterval){
        // 用 Timer 計時輪播
    ...
    /// 停止自動輪播
    public func stopAutoSlideShow(){
        self.timer.invalidate()
    }
    
    /// 更換要顯示到 Sub View
    @objc private func changeScrollSubView(timer : Timer){
        guard
    ...
    private func updateIndex(){
        // 取得現在所在的 index
        let nowPoint = self.contentOffset
        let index = CGFloat(Int(nowPoint.x / self.frame.width))
        self.index = index
    }
}

extension CustomScrollView : UIScrollViewDelegate , CAAnimationDelegate{
